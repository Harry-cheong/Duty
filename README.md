...
Worksheet titles for the planning month (sidebar year/month):

```text
{MMM}{YY} Master Duty Overview   → e.g. JUL26 Master Duty Overview
{MMM}{YY} Send Out               → e.g. JUL26 Send Out
```

Helpers: `master_overview_title(year, month)`, `send_out_title(year, month)`.

### `Personnel List`

Loaded in Step 1. Header row is row 1 (empty header cells are dropped). `NAME`, `RANK & NAME`, and `ORD` are matched **by header name** (not fixed letters). `RANK & NAME` is the stable identity used in availability / planning tables. Clerks with an ORD date before today are skipped when connecting.

| Column | Format |
|--------|--------|
| `NAME` (any column with this header) | Text — short name for matching / selection |
| `RANK & NAME` (any column with this header) | Text — full identity, e.g. `PTE Example Name` |
| `ORD` (must be the **last** column) | `D/M/YYYY` (parsed with `/`); empty allowed |
| D | Lookup key for Send Out VLOOKUPs (`'Personnel List'!D:F`) |
| E | HP NO. (VLOOKUP return col 2) |
| F | BRANCH (VLOOKUP return col 3) |

### `{MMM}{YY} Master Duty Overview`

Created by Step 2 (`create_outline`). Headers in `A1:E2` are merged per column. Clerk data starts at row 3.

| Column | Format |
|--------|--------|
| A — Duty Personnel | Text — clerk `RANK & NAME` |
| B — Total Points | Number / formula |
| C — Clerk | Number / formula — duty count (`SUM` across the slot row) |
| D — R1 | Number / formula — count of `"R"` in that row |
| E — R2 | Number / formula — count of `"R2"` in that row |
| F… — slots | Row 1: day names; row 2: `dd-mm-yy` or `dd-mm-yy (AM/PM)`; row 3+: `1` (duty), `R` (R1), or `R2` (R2) |

Bottom rows after the clerk block hold totals / reserve counts and INDEX-MATCH name helpers across the slot columns.

Step 5 write colours: duty `1` ≈ green, R1 `R` ≈ orange, R2 `R2` ≈ purple. Step 3 availability colours: `0` unavailable ≈ black, `2` preferred ≈ magenta.

### Historical Master sheets (Step 4)

Step 4 reads prior months’ Master Overview sheets from `A3:E100`. Clerks are matched by regex: sheet name **ending with** the personnel `NAME` (rank prefix allowed).

| Column | Format |
|--------|--------|
| A — Clerk name | Text |
| B — (ignored) | — |
| C — Duty points | Number |
| D — R1 points | Number |
| E — R2 points | Number |

### `{MMM}{YY} Send Out`

Created by Step 2 (`create_send_out`). Row 1 is the month title; row 2 is the header row below.

| Column | Format |
|--------|--------|
| A — DAY | Weekday name from slots |
| B — DATE | Slot label (`dd-mm-yy` or with AM/PM) |
| C — CLERK | Text — duty clerk name |
| D — HP NO. | `VLOOKUP` into `'Personnel List'!D:F` |
| E — BRANCH | `VLOOKUP` into `'Personnel List'!D:F` |
| F — STANDBY | Text — R1 name |
| G — HP NO. | `VLOOKUP` for R1 |
| H — BRANCH | `VLOOKUP` for R1 |
| I — STANDBY | Text — R2 name |
| J — HP NO. | `VLOOKUP` for R2 |
| K — BRANCH | `VLOOKUP` for R2 |
